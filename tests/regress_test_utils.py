...
def hack_forward(module: nn.Module, name, io_json, restore=False, keep_tensors=False):
    def _forward(self, *args, **kwargs):
        ...
        if keep_tensors:
            ...
            args = {
                'sum': numpify_tensor_nested(detach_tensor_nested(args), reduction='sum'),
                'mean': numpify_tensor_nested(detach_tensor_nested(args), reduction='mean'),
            }
            kwargs = {
                'sum': numpify_tensor_nested(detach_tensor_nested(kwargs), reduction='sum'),
                'mean': numpify_tensor_nested(detach_tensor_nested(kwargs), reduction='mean'),
            }
            output = {
                'sum': numpify_tensor_nested(detach_tensor_nested(ret), reduction='sum'),
                'mean': numpify_tensor_nested(detach_tensor_nested(ret), reduction='mean'),
            }
...
def compare_arguments_nested(print_content, arg1, arg2, rtol=1.0e-3, atol=1.0e-8):
    ...
    if arg1 is None:
        ...
        arg1 = np.where(np.equal(arg1, None), np.NaN, arg1).astype(dtype=np.float)
        arg2 = np.where(np.equal(arg2, None), np.NaN, arg2).astype(dtype=np.float)
