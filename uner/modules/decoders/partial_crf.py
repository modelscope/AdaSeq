...
class PartialCRF(CRF):
    ...
    def _create_possible_tag_masks(self, num_tags: int,
        ...
        for i in range(1, sequence_length):
            ...
            transition_scores = self.transitions.view(1, num_tags,
                                                      num_tags).expand(
                                                          ...
                                                          num_tags).clone()
            transition_scores[(
                current_possible_tags == 0)] = self.IMPOSSIBLE_SCORE
            transition_scores.transpose(
                1, 2)[(next_possible_tags == 0)] = self.IMPOSSIBLE_SCORE
            ...
            alpha = (
                torch.logsumexp(inner, 1) * mask[i].view(batch_size, 1)
                + alpha * (1 - mask[i]).view(batch_size, 1))

        # Add end transition score
        last_tag_indexes = mask.sum(0).long() - 1
        unnamed = last_tag_indexes + torch.arange(
            batch_size, device=possible_tags.device) * sequence_length
