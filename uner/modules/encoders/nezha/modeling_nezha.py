...
def load_tf_weights_in_nezha(model, config, tf_checkpoint_path):
    ...
    for name, array in zip(names, arrays):
        ...
        if any(n in [
                'adam_v', 'adam_m', 'lamb_m', 'lamb_v',
                'AdamWeightDecayOptimizer', 'AdamWeightDecayOptimizer_1',
...
class RelativePositionsEncoding(nn.Module):
    ...
    def __init__(self, length, depth, max_relative_position=127):
        ...
        embeddings_table = embeddings_table.unsqueeze(0).transpose(
            0, 1).squeeze(1)
        ...
        positions_encoding = torch.matmul(one_hot_relative_positions_matrix,
                                          embeddings_table)
...
class NeZhaSelfAttention(nn.Module):
    ...
    def forward(
        ...
        query_layer_r = query_layer_t.contiguous().view(
            ...
            self.attention_head_size)
        key_position_scores = torch.matmul(query_layer_r,
                                           relations_keys.permute(0, 2, 1))
        ...
        value_position_scores = torch.matmul(attentions_probs_r,
                                             relations_values)
...
class NeZhaPreTrainedModel(PreTrainedModel):
    ...
    def save_pretrained(
        ...
        state_dict = {
            k: v
            for k, v in state_dict.items() if 'relative_positions' not in k
        }
        ...
        if from_tf:
            if resolved_archive_file.endswith('.index'):
                ...
                try:
                    ...
                    logger.error(
                        'Loading a TensorFlow model in PyTorch, requires both PyTorch and TensorFlow to be installed. Please see '  # noqa
            ...
            if len(unexpected_keys) > 0:
                unexpected_keys = [
                    k for k in unexpected_keys if 'relative_positions' not in k
                ]
                logger.warning(
                    f'Some weights of the model checkpoint at {pretrained_model_name_or_path} were not used when '
                    f'initializing {model.__class__.__name__}: {unexpected_keys}\n'
                    f'- This IS expected if you are initializing {model.__class__.__name__} from the checkpoint of a model trained on another task '  # noqa
                    f'or with another architecture (e.g. initializing a BertForSequenceClassification model from a BertForPreTraining model).\n'  # noqa
                    f'- This IS NOT expected if you are initializing {model.__class__.__name__} from the checkpoint of a model that you expect '  # noqa
                    f'to be exactly identical (initializing a BertForSequenceClassification model from a BertForSequenceClassification model).'  # noqa
                )
            else:
                logger.info(
            ...
            if len(missing_keys) > 0:
                missing_keys = [
                    k for k in missing_keys if 'relative_positions' not in k
                ]
                logger.warning(
                    f'Some weights of {model.__class__.__name__} were not initialized from the model checkpoint at {pretrained_model_name_or_path} '  # noqa
                    f'and are newly initialized: {missing_keys}\n'
                    f'You should probably TRAIN this model on a down-stream task to be able to use it for predictions and inference.'  # noqa
                )
            else:
                logger.info(
...
class NeZhaModel(NeZhaPreTrainedModel, Encoder):
    ...
    @classmethod
    def _instantiate(cls, model_name_or_path=None, **kwargs):
        if model_name_or_path is not None:
            ...
            config = NeZhaConfig()
            config.output_hidden_states = True
            return NeZhaModel(config=config)
