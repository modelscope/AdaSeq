...
class SequenceLabelingModel(Model):
    ...
    def _forward(self, inputs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'emission_mask' in inputs:
            ...
            for i in range(len(mask)):
                masked_logits[i, :masked_lengths[i], :] = logits[i].masked_select(mask[i].unsqueeze(-1)).view(
                    masked_lengths[i], -1)
    ...
    def forward(self, inputs: Dict[str, Any]) -> Dict[str, Any]:  # noqa
        ...
        mask = torch.arange(inputs['emission_mask'].shape[1], device=seq_lens.device)[None, :] < seq_lens[:, None]
        ...
        if self.training:
            ...
            if self.multiview:  # for multiview training
                ...
                if self.mv_loss_type == 'kl':
                    cl_kl_loss = self._calculate_cl_loss(logits, origin_view_logits, mask, T=self.temperature)
                    loss = self.mv_interpolation * (loss + origin_view_loss) + (1 - self.mv_interpolation) * cl_kl_loss
    ...
    def _calculate_loss(self, logits: torch.Tensor, targets: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        if self.use_crf:
